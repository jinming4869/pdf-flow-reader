...
## 2026-08-16：beta.3 分栏阅读顺序修复复核

产品所有者指出：分栏论文的阅读顺序错误，同一视觉行左右栏被串行朗读而非先左栏后右栏。复核结论：

- 旧切分逻辑用固定页面中线加严格不等式猜测栏沟，IEEE 式版面（左栏右缘恰在中线 306/612）与窄栏距期刊（14-24pt 远小于 36pt 绝对阈值）全部切分失败，左右栏合并为一行，两栏判定退化为按 y 串行；
- 修复改为整页栏边界检测：每行最大空隙的中位宽度、位置覆盖占比与起点偏差共同确认栏沟；切分按边界执行；跨栏居中标题排在左栏之前；点读列流共用同一检测；
- 最小栏距 14pt，单栏 justified 文本在中线附近的大词距不再误判为栏沟。

beta.3 证据：

- Node 回归 376 / 376（新增 8 个双栏场景，含 IEEE colwise / interleaved、窄栏距、短行、跨栏标题、基线漂移、单栏 justified 反例）；
- 真实双栏 PDF 端到端：左栏 40 行全部在前、右栏 40 行全部在后；「流觞曲水」档左栏一个 passage、右栏一个 passage，column-change 边界正确；
- packaged 中、日、英 TTS 与硬取消通过，ad-hoc deep sign + strict verify 通过；
- beta.3 ZIP SHA-256 为 `221d9160d927807fda1459949ce84cc755b7e223d38e8f40831b64ca4eb0fade`。

残余限制：栏距小于 14pt 的版面回退按行串读；三栏论文不在本次范围。真实论文听感仍需产品所有者复测。

## 2026-08-16：stage 10 退出门关闭

产品所有者完成 beta.3 复测与五类真实 PDF（普通、双栏、扫描、旋转、大型）离线端到端及 30 / 90 秒回流主观验收，结论通过。返回书架、最低两档连续朗读与分栏阅读顺序三项反馈全部关闭。

- 产品所有者确认分栏论文已先左栏后右栏朗读；
- 产品所有者确认返回书架与最低两档朗读听感正常；
- 产品所有者确认 30 / 90 秒回流主观体验可接受。

stage 10 退出门关闭，v5.0 离线核心进入待发布状态。发布策略更新：全系列（v5.1–v5.3）完成后统一打 tag 与 GitHub Release，此前每版只产出本机验收候选，不单独发布。
