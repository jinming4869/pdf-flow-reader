...
## v5.0.0-beta.3 分栏阅读顺序修复（2026-08-16）

基线：`c40891f`（含 `f97050e` 分栏修复）

```text
night-study-5.0.0-beta.3-mac-arm64.zip
SHA-256 221d9160d927807fda1459949ce84cc755b7e223d38e8f40831b64ca4eb0fade
```

- 分栏论文改为先左栏后右栏；旧逻辑的固定中线严格不等式在 IEEE 式版面（左栏右缘恰在中线）和窄栏距期刊上切分失败，左右栏合并成一行串行朗读。
- 新增整页栏边界检测（每行最大空隙的中位宽度、覆盖占比、起点偏差），跨栏居中标题排在左栏之前。
- 最小栏距 14pt；单栏正文在中线附近的大词距不再误判为栏沟。
- Node 回归 376 / 376；真实双栏 PDF 端到端验证左栏 40 行在前、右栏 40 行在后；packaged 三语言 TTS 与硬取消通过；ad-hoc deep sign + strict verify 通过。
- 残余限制：栏距小于 14pt 的版面仍回退到按行串读；三栏论文不在本次范围。
