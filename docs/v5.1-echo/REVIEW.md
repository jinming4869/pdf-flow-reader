...
## 2026-08-16：v5.1 验收通过

产品所有者完成 v5.1.0-beta.1 真实 PDF 验收：未配置降级、凭据与复述、归档导出、六档朗读与分栏顺序回归全部通过。

- 风险 V1-R3（迟到结果覆盖/重复发送）关闭：echo-state 按 traceId 终态去重，pending 不重复生成；
- 风险 V1-R7（endpoint 配置误用）关闭：配置契约仅 baseURL / 模型 / 视觉开关；
- 风险 V1-R6（私人内容进 AI）部分关闭：默认只发圈选文字与裁图、不发全文与情绪；“发送前逐项授权”未实现，产品所有者验收接受“配置即一次性授权”；
- v5.1「回声与归档」完成，进入 v5.2 需求冻结。
